(function () {
    ...
    angular.module('goku.modal')
        ...
        .controller('Common_SingelInputAndTextareaModalCtrl', Common_SingelInputAndTextareaModalCtrl)
    ...
    Common_SingelInputAndTextareaModalCtrl.$inject = ['$scope', '$rootScope','$uibModalInstance','input'];

    function Common_SingelInputAndTextareaModalCtrl($scope, $rootScope, $uibModalInstance,input) {
    ...
    Common_LoginModalCtrl.$inject = ['$scope', 'CODE', 'COOKIE_CONFIG', '$rootScope', '$cookies', 'CommonResource', '$uibModalInstance'];

    function Common_LoginModalCtrl($scope, CODE, COOKIE_CONFIG, $rootScope, $cookies,CommonResource, $uibModalInstance) {
        $scope.data = {
            info: {
                submitted: false
            },
        ...
        $scope.data.fun.confirm = function () {
            var template = {
                request: {
                    loginName: $scope.data.interaction.request.loginName,
                    loginPassword: CryptoJS.MD5($scope.data.interaction.request.loginPassword).toString(),
                    verifyCode: CryptoJS.MD5((new Date()).toUTCString()).toString()
                }
            }
                        ...
                        case CODE.COMMON.SUCCESS:
                            {
                                $uibModalInstance.close({
                                    loginName: template.request.loginName
                                });
                                break;
                            }
                        default:
