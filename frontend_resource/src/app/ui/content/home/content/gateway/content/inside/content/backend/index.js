(function() {
    ...
    angular.module('goku')
        ...
        .component('gatewayBackend', {
            templateUrl: 'app/ui/content/home/content/gateway/content/inside/content/backend/index.html',
    ...
    function indexController($scope, GatewayResource, $state, CODE, $rootScope, $timeout, Cache_CommonService) {
        ...
        vm.data.fun.edit = function(arg) {
            ...
            $rootScope.GatewayBackendModal(template.modal.title, arg.item, function(callback) {
                if (callback) {
                    callback.gatewayHashKey = vm.data.interaction.request.gatewayHashKey;
                    callback.backendURI = callback.backendURI.replace(/(http:\/\/)|(https:\/\/)/, '');
                    callback.backendURI = callback.backendURI.replace(/\/*\/$/, '');
                    if (arg.item) {
                                    ...
                                    case CODE.COMMON.SUCCESS:
                                        {
                                            ...
                                            arg.item.backendURI = callback.backendURI;
                ...
                mainObject: {
                    item: {
                        default: [
                            ...
                            { key: '后端域名/IP地址', html: '{{item.backendURI}}', keyStyle: { 'min-width': '400px' } },
                        ],
                        fun: {
                            array: [
