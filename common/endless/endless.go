...
func (srv *endlessServer) Serve() (err error) {
	defer log.Info(syscall.Getpid(), "Serve() returning...")
	srv.setState(STATE_RUNNING)
	err = srv.Server.Serve(srv.EndlessListener)
	log.Info(syscall.Getpid(), "Waiting for connections to finish...")
	srv.wg.Wait()
...
func (srv *endlessServer) ListenAndServeTLS(certFile, keyFile string) (err error) {
	...
	log.Info(syscall.Getpid(), srv.Addr)
	return srv.Serve()
}
...
func (srv *endlessServer) getListener(laddr string) (l net.Listener, err error) {
	if srv.isChild {
		...
		if len(socketPtrOffsetMap) > 0 {
			...
			log.Info("laddr", laddr, "ptr offset", socketPtrOffsetMap[laddr])
		}
...
func (srv *endlessServer) handleSignals() {
	...
	for {
		...
		switch sig {
		case syscall.SIGHUP:
			log.Info(pid, "Received SIGHUP. forking.")
			err := srv.fork()
			if err != nil {
				log.Info("Fork err:", err)
			}
			isStop = false
		case syscall.SIGUSR1:
			log.Info(pid, "Received SIGUSR1.")
		case syscall.SIGUSR2:
			log.Info(pid, "Received SIGUSR2.")
		...
		default:
			log.Infof("Received %v: nothing i care about...\n", sig)
		}
...
func (srv *endlessServer) shutdown() {
	...
	if err != nil {
		log.Info(syscall.Getpid(), "Listener.Close() error:", err)
	} else {
		log.Info(syscall.Getpid(), srv.EndlessListener.Addr(), "Listener closed.")
	}
}
...
func (srv *endlessServer) hammerTime(d time.Duration) {
	...
	time.Sleep(d)
	log.Info("[STOP - Hammer Time] Forcefully shutting down parent")
	for {
